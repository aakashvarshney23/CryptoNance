...
let litecoin_val = 154;

@IonicPage()
@Component({
    selector: 'page-litecoinpage',
    templateUrl: 'litecoinpage.html',
})
export class LitecoinPage { //sell key = 1;
    ...
    constructor(private alertCtrl: AlertController,
        ...
        this.walletService.getWallets().subscribe((wallet) =>{
            ...
            this.litecoinamount = this.wallet.litecoinamount;
            this.totalamount = this.wallet.totalamount;
            this.litecoinamount2 = this.wallet2.litecoinamount;
    ...
    sell(){
        if((+this.sellCoins.value * litecoin_val) > this.litecoinamount || (+this.sellCoins.value * litecoin_val) > this.totalamount2){
            ...
            let A = Math.abs(+this.sellCoins.value * litecoin_val);
            this.addTrans(T, A);
    ...
    buy(){
        if((+this.buyCoins.value * litecoin_val) > this.totalamount || (+this.buyCoins.value * litecoin_val) > this.litecoinamount2){
            ...
            let T = Math.floor(Math.random() * (9000 - 1000 + 1)) + 1000;
            let A = -Math.abs(+this.buyCoins.value * litecoin_val);
            this.addTrans(T, A);
