...
let ethereum_val = 755;

@IonicPage()
@Component({
    selector: 'page-ethereumpage',
    templateUrl: 'ethereumpage.html',
})
export class EthereumPage { //sell key = 1;
    ...
    constructor(private alertCtrl: AlertController,
        ...
        this.walletService.getWallets().subscribe((wallet) =>{
            ...
            this.ethereumamount = this.wallet.ethereumamount;
            this.totalamount = this.wallet.totalamount;
            this.ethereumamount2 = this.wallet2.ethereumamount;
    ...
    sell(){
        if((+this.sellCoins.value * ethereum_val) > this.ethereumamount || (+this.sellCoins.value * ethereum_val) > this.totalamount2){
            ...
            let A = Math.abs(+this.sellCoins.value * ethereum_val);
            this.addTrans(T, A);
    ...
    buy(){
        if((+this.buyCoins.value * ethereum_val) > this.totalamount || (+this.buyCoins.value * ethereum_val) > this.ethereumamount2){
            ...
            let T = Math.floor(Math.random() * (9000 - 1000 + 1)) + 1000;
            let A = -Math.abs(+this.buyCoins.value * ethereum_val);
            this.addTrans(T, A);
